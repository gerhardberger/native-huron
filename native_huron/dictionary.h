...
#ifndef NATIVE_HURON_DICTIONARY_H_
#define NATIVE_HURON_DICTIONARY_H_
...
class Dictionary {
 public:
  ...
  template<typename T>
  bool GetHidden(const std::string& key, T* out) const {
    v8::Local<v8::Context> context = isolate_->GetCurrentContext();
  ...
  template<typename T>
  bool SetHidden(const std::string& key, T val) {
    v8::Local<v8::Value> v8_value;
    if (!TryConvertToV8(isolate_, val, &v8_value))
      return false;
    v8::Local<v8::Context> context = isolate_->GetCurrentContext();
  ...
  template<typename T>
  bool SetMethod(const std::string& key, const T& callback) {
...
#endif  // NATIVE_HURON_DICTIONARY_H_
